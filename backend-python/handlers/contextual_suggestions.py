from __future__ import annotations
import logging

from fastapi import APIRouter, Depends, Request
from fastapi.responses import JSONResponse
from sqlalchemy.ext.asyncio import AsyncSession

from database import get_db
from models.api import ContextualSuggestionsRequest, SuggestionsResponse
from services import dataset_service, gemini
from utils.response import error, success

logger = logging.getLogger(__name__)
router = APIRouter()


@router.post("/contextual-suggestions")
async def get_contextual_suggestions(
    request: Request, body: ContextualSuggestionsRequest, db: AsyncSession = Depends(get_db)
) -> JSONResponse:
    req_id = getattr(request.state, "request_id", None)
    try:
        record = await dataset_service.get_dataset_record(db, body.file_id)
        if record is None:
            return error("Dataset not found. Please re-upload your file.", "NOT_FOUND", req_id, 404)

        # Schema-only — no need to re-fetch the file from storage here.
        columns, summary = dataset_service.schema_from_record(record)
        suggestions = await gemini.get_contextual_suggestions(columns, summary, body.recent_chats)
        return success(SuggestionsResponse(suggestions=suggestions))

    except Exception as exc:
        logger.exception("Contextual suggestions failed")
        return error(str(exc), "INTERNAL_ERROR", req_id, 500)
